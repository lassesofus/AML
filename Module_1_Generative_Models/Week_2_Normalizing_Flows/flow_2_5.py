...
class MaskedCouplingLayer(nn.Module):
    ...
    def inverse(self, x):
        ...
        log_det_J = torch.sum((1 - self.mask) * -self.scale_net(self.mask * x), dim=1) # Here, we multiply with the negative scale
        return z, log_det_J
...
def train(model, optimizer, data_loader, epochs, device, output_dir):
    ...
    for epoch in range(epochs):
        ...
        for batch in data_iter:
            x, _ = batch  # Extract the data part from the tuple
            x = x.to(device)
            optimizer.zero_grad()
            loss = model.loss(x)
            loss.backward()
...
if __name__ == "__main__":
    ...
    for i in range(num_transformations):
        if args.masking == 'random':
            ...
            mask = 1 - mask  # Flip the mask for chequerboard at each layer
